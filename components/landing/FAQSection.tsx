...
export default function FAQSection() {
  ...
  return (
    <section className="py-32 bg-white dark:bg-[#0B0B0B] text-black dark:text-white">
      <div className="container mx-auto px-6 max-w-4xl">
        <div className="text-center mb-24 space-y-4">
          ...
          <h2 className="text-4xl md:text-7xl font-black italic uppercase tracking-tighter leading-none">FAQS.</h2>
        </div>
        ...
        <div className="space-y-4">
          {faqs.map((f, i) => (
            <div 
              key={i} 
              className="border-b border-zinc-100 dark:border-zinc-900 overflow-hidden"
            >
              <button 
                onClick={() => setOpen(open === i ? null : i)}
                className="w-full py-10 flex items-center justify-between text-left group"
              >
                <span className="text-lg md:text-2xl font-black italic uppercase tracking-tighter group-hover:text-red-500 transition-colors">
                  {f.q}
                </span>
                <div className={`w-10 h-10 rounded-full bg-zinc-50 dark:bg-zinc-900 flex items-center justify-center transition-all ${open === i ? 'rotate-45 bg-red-500 text-white' : ''}`}>
                  <FiPlus size={20} />
                </div>
              </button>
              ...
              <AnimatePresence>
                {open === i && (
                  ...
                  >
                    <p className="pb-10 text-sm md:text-lg italic font-medium text-zinc-500 dark:text-zinc-400 leading-relaxed max-w-2xl">
                      {f.a}
                    </p>
                  </motion.div>
                )}
